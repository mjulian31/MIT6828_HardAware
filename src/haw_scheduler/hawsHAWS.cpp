...
std::map<std::string, long> CPUTimeMap;
std::map<std::string, long> GPUTimeMap;
...
void HAWS::DispatchConclusion(pid_t pid, TaskStatus task_status, int status, time_point ended) {
    ...
    if (IN_LIST(allCPUPids, pid)) {
       ...
       wallTime = std::stol(conclusion->wallTime);
...
void HAWS::ScheduleLoop(int cpuThreadLimit, int gpuThreadLimit,
    ...
    while (!schedLoopKillFlag) {
        ...
        if (throttle++ % 5000 == 0) {
            ...
            // print dictionaries
            for(map<string, long>::const_iterator it = CPUTimeMap.begin(); it != CPUTimeMap.end(); ++it) {
                printf("task id: %s -> cpu: %ld\n", (it->first).c_str(), it->second);
            }
            printf("\n");
            for(map<string, long>::const_iterator it = GPUTimeMap.begin(); it != GPUTimeMap.end(); ++it) {
...
HAWSHWTarget HAWS::DetermineReqTarget(HAWSClientRequest* req) { // SCHEDLOOP THREAD
    ...
    if (GPUTimeMap.count(req->jobID) > 0) {
      // in map
      estGPU = GPUTimeMap[req->jobID];
    }
    long estCPU = LONG_MAX;
    if (CPUTimeMap.count(req->jobID) > 0) {
      // in map
      estCPU = CPUTimeMap[req->jobID];
    }
    if (hint == "cpu-please" && estCPU <= estGPU) return HAWS::RandomizeTarget(TargCPU, 0.75);
    if (hint == "gpu-please" && estGPU <= estCPU) return HAWS::RandomizeTarget(TargGPU, 0.75);
    if (hint == "any" && (estCPU == FLOAT_MAX || estGPU == FLOAT_MAX)) return HAWS::RandomizeTarget(TargGPU, 0.50);
    if (hint == "any") return (estCPU <= estGPU)? HAWS::RandomizeTarget(TargCPU, 0.65): HAWS::RandomizeTarget(TargGPU, 0.65);

    return (estCPU <= estGPU)? HAWS::RandomizeTarget(TargCPU, 0.65): HAWS::RandomizeTarget(TargGPU, 0.65);
}
